...
class RAEIOAgent:
    def __init__(self, config=None, logger=None):
        ...
        # Memory and caches
        self.memory = TaskMemory(path=self.config.get("memory_path", "task_memory.jsonl"))
        self.cache_manager = CacheManager(
            temp_dir=self.config.get("temp_dir", "temp"),
            cache_dir=self.config.get("cache_dir", "cache"),
            max_temp_mb=self.config.get("max_temp_mb", 500),
            max_cache_mb=self.config.get("max_cache_mb", 500),
            check_interval=self.config.get("cache_check_interval", 300),
            logger=logger
        )
        self.cache_manager.start_auto_clean()

        # Plugins and external systems
        self.plugin_registry = PluginRegistry(plugin_dir=self.config.get("plugin_dir", "plugins"), logger=logger)
        self.tts_manager = TTSManager(
            voice=self.config.get("tts_voice", "tts_models/en/vctk/vits"),
            cache_dir=self.config.get("tts_cache_dir", "tts_cache"),
            logger=logger
        )
        self.browser_automation = BrowserAutomation(
            user_agent=self.config.get("browser_user_agent"),
            proxy=self.config.get("browser_proxy"),
    ...
    # ------------------------------------------------------------------
    # Encryption helpers for Fuckery mode
    # ------------------------------------------------------------------
